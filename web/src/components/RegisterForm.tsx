...
export function RegisterForm({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setDone({
        message: res.verification_token ? t("registro.verified") : res.message || t("registro.checkYourEmail"),
        token: res.verification_token ?? null,
      });
      // Auto-login tras registrar: sin sesión, la intención (p. ej. donación) no se reanuda
      // y el usuario vuelve a "registrarse" en bucle.
      try {
  ...
  if (done !== null) {
    return (
      <div className="flex flex-col gap-2">
        <p className="text-sm text-osap-muted">{done.message}</p>
        {loginFailed && onSwitchToLogin ? (
          <button
            type="button"
            onClick={onSwitchToLogin}
            className="self-start rounded border border-osap-accent px-3 py-1 text-sm text-osap-accent"
          >
            {t("auth.login")}
          </button>
        ) : null}
